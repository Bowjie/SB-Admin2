            ...
            <div class="col-md-4 col-md-offset-4">
                <div class="login-panel panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <?php if(isset($_POST['email'])){ $eml=$_POST['email'];}
                              if(isset($_POST['password'])){ $pwd=$_POST['password'];} ?>

                        <form role="form" action="<?=site_url('sbadmin2/home');?>" method="post">
                            <fieldset>
                                    <?= $one = $this->session->flashdata('error');?>
                                <div class="form-group">
                                    <input class="form-control" placeholder="E-mail" name="email" type="text" autofocus value="<?=@$eml; echo $two = $this->session->flashdata('error_email');?>">
                                    <span class="text-danger"><?= form_error('email');?></span>
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="Password" name="password" type="password" value="<?=@$pwd; echo $three = $this->session->flashdata('error_pwd');?>">
